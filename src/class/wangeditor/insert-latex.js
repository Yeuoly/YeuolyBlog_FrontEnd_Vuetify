...
let img_index = 0;
//渲染dom中的latex
const renderLatex = (dom, handle_img) => {
    for(let i of dom.children){
        if(i.tagName === 'LATEXPRESSION'){
            ...
            renderLatex(i, handle_img);
        }
    }
}

//解析包含latex表达式的html，可以是字符串也可以是dom，返回dom
export const analysisLatex = (html, handle_img) => {
    let dom = null;
...
const createPanel = (editor, selector) => {
